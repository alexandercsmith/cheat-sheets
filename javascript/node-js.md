...
console.log('Server running at http://127.0.0.1:7766');
```
...
[Node.js - Globals](http://nodejs.org/api/globals.html)

Node.js Global Objects and References

```javascript
// Absolute Path
__filename;

// Name of Directory which current execution resides
__dirname;
...
// Global object to reference `EventEmitter`
process;
...
[Node.js - Console](http://nodejs.org/api/console.html)

Console Logging

```javascript
console.log([data], [...]);            // stdout to new line
console.info([data], [...]);           // same as console.log
console.error([data], [...]);          // stderr to new line
console.warn([data], [...]);           // same as console.error
console.dir(obj);                      // uses util.inspect on object
console.time(label);                   // mark a time
console.timeEnd(label);                // finish timer
console.trace(label);                  // print stack trace to stderr
console.assert(expression, [message]); // same as assert.ok() AssertionError with message
```
...
```javascript
// One-time callback after delay milliseconds
setTimeout(callback, delay, [arg], [...]);

// Stop a timer once created with setTimeout()
clearTimeout(t);

// Repeated callback after delay milliseconds
setInterval(callback, delay, [arg], [...]);

// Stop a timer once created with setInterval()
clearInterval(t);

// Execution callback after I/O events callbacks and before setTimeout and setInterval
setImmediate(callback, [arg], [...]);

// Stop a timer once created with setImmediate()
clearImmediate(immediateObject);

// Create a timer that is active but if it is the only item left in the event loop
unref();

// Previously unref()d a timer you can call ref() to explicitly request the timer hold the program open
ref();
```
...
// Load Module from another Module
module.require('./another_module.js');

module.id;        // The identifier for the module.
module.filename;  // Fully resolved filename to the module.
...
[Node.js - Process](http://nodejs.org/api/process.html)
...
[Node.js - Child Process](http://nodejs.org/api/child_process.html)
...
[Node.js - Util](http://nodejs.org/api/util.html)
...
[Node.js - File System](http://nodejs.org/api/fs.html)
...
[Node.js - Path](http://nodejs.org/api/fs.html)
...
[Node.js - HTTP](http://nodejs.org/api/http.html)
...
[Node.js - Query String](http://nodejs.org/api/querystring.html)
...
[Node.js - OS](http://nodejs.org/api/os.html)
